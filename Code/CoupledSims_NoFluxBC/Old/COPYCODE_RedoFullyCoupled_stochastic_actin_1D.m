...
% Set feedback
%
alpha   = 0.5;                  % feedback from Rac/Rho to actin
beta    = 0.0;                  % feedback from actin to Rac/Rho
...
% Set movie making
%
%vidObj = VideoWriter('sim_fullycoupled05_switch_05.avi');
%vidObj.FrameRate = 5;
%open(vidObj);
...
%
figure(1); 
plot(Xa,a,'-ob','markerfacecolor','b','linewidth',2); hold on;
...
cond = 1; % switch in external gradient cue
...
for t=1:(Nt-1)
    ...
    %% Run biochemisty
    [Konx,Kony,Kfbx,Kfby,Koffx,Koffy] = spatialrates(ron,rfb,roff,a,b,s,cond); % set rates
    ...
    ponx = kon/(kon+kfb*(N-K1));
    pony = kon/(kon+kfb*(N-K2));
...
%     end
    ...
    %if t==(Nt-1)
        ...
        legend('Branched network','Contractile network','Location','northeast');
        drawnow;
        hold off;
