...
while (ppp<=1)
    ...
    z_stable_idx1 = zeros(N,Nt);      % 1 means stable, 0 means unstable; corresponds to indices of posz1
    ...
    z_stable_idx2 = zeros(N,Nt);      % 1 means stable, 0 means unstable; corresponds to indices of posz2
    ...
    epsilon2 = 0.5;
    boundzC1scaled = boundzC1*L/Na;
    boundzC2scaled = boundzC2*L/Na;
    for i=1:NNz1(1)
        if (posz1(i,1) >= boundzC1scaled(1)) && (posz1(i,1) <= boundzC1scaled(end)) && (~isempty(find(abs(posz1(i,1)-(L-posz2(1:NNz2(1),1)))<=epsilon2,1)))
            z_stable_idx1(i,1)=1;
        else
            z_stable_idx1(i,1)=0;
        end
    ...
    for i=1:NNz2(1)
        if (posz2(i,1) >= boundzC2scaled(1)) && (posz2(i,1) <= boundzC2scaled(end)) && (~isempty(find(abs(posz2(i,1)-(L-posz1(1:NNz1(1),1)))<=epsilon2,1)))
            z_stable_idx2(i,1)=1;
        else
            z_stable_idx2(i,1)=0;
        end
    ...
    posz11 = posz1(1:NNz1(1),1);
    z_stable_idx11 = z_stable_idx1(1:NNz1(1),1);
    posz22 = posz2(1:NNz2(1),1);
    z_stable_idx22 = z_stable_idx2(1:NNz2(1),1);
    ...
    for t=1:(Nt-1)
        ...
        if (NNz1(t+1) < NNz1(t))                % diassociation event (particle off)
            ...
            oldcol_stable = z_stable_idx1(minidz1,1:end);
            othercols = posz1([1:minidz1-1,minidz1+1:K3_1],1:end);
            othercols_stable = z_stable_idx1([1:minidz1-1,minidz1+1:K3_1],1:end);
            otherothercols = posz1(K3_1+1:end,1:end);
            otherothercols_stable = z_stable_idx1(K3_1+1:end,1:end);
            newpos = [othercols;oldcol;otherothercols];
            newpos_stable = [othercols_stable;oldcol_stable;otherothercols_stable];
            posz1 = newpos;
            z_stable_idx1 = newpos_stable;
            nz1(K3_1,t+1) = 0;
            z_stable_idx1(K3_1,t+1) = 0;
        ...
        if (NNz2(t+1) < NNz2(t))                % diassociation event (particle off)
            ...
            oldcol_stable = z_stable_idx2(minidz2,1:end);
            othercols = posz2([1:minidz2-1,minidz2+1:K3_2],1:end);
            othercols_stable = z_stable_idx2([1:minidz2-1,minidz2+1:K3_2],1:end);
            otherothercols = posz2(K3_2+1:end,1:end);
            otherothercols_stable = z_stable_idx2(K3_2+1:end,1:end);
            newpos = [othercols;oldcol;otherothercols];
            newpos_stable = [othercols_stable;oldcol_stable;otherothercols_stable];
            posz2 = newpos;
            z_stable_idx2 = newpos_stable;
            nz2(K3_2,t+1) = 0;
            z_stable_idx2(K3_2,t+1) = 0;
        ...
        boundzC2scaled = boundzC2*L/Na;
        for i=1:NNz1(t+1)
            if posz1(i,t+1) >= boundzC1scaled(1) && posz1(i,t+1) <= boundzC1scaled(end) && ~isempty(find(abs(posz1(i,t+1)-(L-posz2(1:NNz2(t+1),t+1)))<=epsilon2,1))
                z_stable_idx1(i,t+1)=1;
            else
                z_stable_idx1(i,t+1)=0;
            end
        end
        for i=1:NNz2(t+1)
            if posz2(i,t+1) >= boundzC2scaled(1) && posz2(i,t+1) <= boundzC2scaled(end) && ~isempty(find(abs(posz2(i,t+1)-(L-posz1(1:NNz1(t+1),t+1)))<=epsilon2,1))
                z_stable_idx2(i,t+1)=1;
            else
                z_stable_idx2(i,t+1)=0;
            end
        end
        ...
        % if t==(Nt-1)
            ...
            posz11 = posz1(1:K3_1,t+1);
            z_stable_idx11 = z_stable_idx1(1:K3_1,t+1);
            posz22 = posz2(1:K3_2,t+1);
            z_stable_idx22 = z_stable_idx2(1:K3_2,t+1);
