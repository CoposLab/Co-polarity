    ...
    elseif isempty(dirIndex1) || isempty(dirIndex2)
        ...
        if xor(abs(medang1-3*pi/2)<angle, abs(medang2-pi/2)<angle)
            ...
            if abs(medang1-3*pi/2)<angle %if cell 2 is the leader
                if medang2>=0 && medang2<=pi
                    countleft=countleft+1;
                elseif medang2>=pi && medang2<=2*pi
                    countright=countright+1;
                end
            else %otherwise cell 1 is the leader
                if medang1>=0 && medang1<=pi
                    countleft=countleft+1;
                elseif medang1>=pi && medang1<=2*pi
                    countright=countright+1;
                end
            end
        end
...
sprintf(['%d yes, %d strong no, %d 1NP, %d 2NP\nNumber leader/follower: %d\n' ...
    ...
    counteryes, countersn, counter1np, counter2np, counterlf,counterdist,signal_polarized,both_sig_pol,countleft,countright)
