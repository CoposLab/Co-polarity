...
% rac rho parameter search
 % for c1_ind=1:4 %koffx,koffy,konx,kony
 %    for c2_ind=1:4 %koffx,koffy,konx,kony
 %        for c1coeff_ind=1:3 %1,10,1000
...
while (ppp<=100)
    ...
    savefigs=1;
    setnum=int2str(ppp);
    savelocation='./simulation_results/results_nosignal/branchedbundledpromotion3/0_75ka0_75kb2alpha50max';
    if savefigs==1
    ...
    % Set rac rho coupling
    %
    % multiply rac rho binding and unbinding rates by a constant
    bindunbind_rates=0; % 1 for coupled (use coeffs below), 0 for uncoupled (all coeffs are 1)
    %
    % coefficients for each binding/unbinding rate in contact region
    % ex: Konx1=racon1*Konx1 in contact region
    racon1=1;
    racon2=1;
    rhoon1=1;
    rhoon2=1;
    ...
    rhooff1=1;
    rhooff2=1;
    % coefficients for away from contact region
    racon1_away=1;
    racon2_away=1;
    rhoon1_away=1;
    rhoon2_away=1;
    ...
    % binding and unbinding based on other rac/rho amounts nearby (set
    % below)
    racrho_crosscells_coupled=0; % 0 is uncoupled, 1 is coupled


    % rac rho based on branched and bundled concentrations in the other
    % cell at the contact region
    bindunbind_forces=0; % 1 is coupled (use coeffs below), 0 is uncoupled (all coeffs are 1)
    % binding rates: Konx1=Konx1*b2*rac1b2, etc
    rac1b2=1;
    rac2b1=1;
    rho1a2=1;
    rho2a1=1;
    % unbinding rates: Koffx1=Koffx1*a2*rac1a2, etc
    rac1a2=1;
    rac2a1=1;
    rho1b2=1;
    rho2b1=1;
    ...
    for t=1:(Nt-1)
        ...
        if signal==1
         ...
         Konx1(boundC1)=Konx1(boundC1)*racon1;
         Konx2(boundC2)=Konx2(boundC2)*racon2;
         Kony1(boundC1)=Kony1(boundC1)*rhoon1;
         Kony2(boundC2)=Kony2(boundC2)*rhoon2;
         Koffx1(boundC1)=Koffx1(boundC1)*racoff1;
         Koffx2(boundC2)=Koffx2(boundC2)*racoff2;
         Koffy1(boundC1)=Koffy1(boundC1)*rhooff1;
         Koffy2(boundC2)=Koffy2(boundC2)*rhooff2;

         % Set konx and kony away from contact region
         Konx1(setdiff(1:length(Konx1),boundC1)) = Konx1(setdiff(1:length(Konx1),boundC1))*racon1_away;
         Konx2(setdiff(1:length(Konx2),boundC2)) = Konx2(setdiff(1:length(Konx2),boundC2))*racon2_away;
         Kony1(setdiff(1:length(Kony1),boundC1)) = Kony1(setdiff(1:length(Kony1),boundC1))*rhoon1_away;
         Kony2(setdiff(1:length(Kony2),boundC2)) = Kony2(setdiff(1:length(Kony2),boundC2))*rhoon2_away;
         Koffx1(setdiff(1:length(Koffx1),boundC1)) = Koffx1(setdiff(1:length(Koffx1),boundC1))*racoff1_away;
         Koffx2(setdiff(1:length(Koffx2),boundC2)) = Koffx2(setdiff(1:length(Koffx2),boundC2))*racoff2_away;
         Koffy1(setdiff(1:length(Koffy1),boundC1)) = Koffy1(setdiff(1:length(Koffy1),boundC1))*rhooff1_away;
         Koffy2(setdiff(1:length(Koffy2),boundC2)) = Koffy2(setdiff(1:length(Koffy2),boundC2))*rhooff2_away;
         


        % Set konx and kony depending on rac/rho concentrations in contact
        % region
        if racrho_crosscells_coupled==1
            epsilon1 = 0.1;
            flipc2=flip(boundC2);
        ...
        if bindunbind_forces==1
            Konx1(boundC1) = Konx1(boundC1).*flip(b2(boundC2))*rac1b2;
            Konx2(boundC2) = Konx2(boundC2).*flip(b1(boundC1))*rac2b1;
            Kony1(boundC1) = Kony1(boundC1).*flip(a2(boundC2))*rho1a2;
            Kony2(boundC2) = Kony2(boundC2).*flip(a1(boundC1))*rho2a1;
            Koffx1(boundC1) = Koffx1(boundC1).*flip(a2(boundC2))*rac1a2;
            Koffx2(boundC2) = Koffx2(boundC2).*flip(a1(boundC1))*rac2a1;
            Koffy1(boundC1) = Koffy1(boundC1).*flip(b2(boundC2))*rho1b2;
            Koffy2(boundC2) = Koffy2(boundC2).*flip(b1(boundC1))*rho2b1;
        end
        ...
        if countpol==1
            if polarizedc1==0 && ~isempty(dirIndexa1) && ~isempty(dirIndexb1)
                ...
                if polc1_counter>=100
                    ...
                    polarize_time_c1=polarize_time_c1+t-100;
            ...
            if polarizedc2==0 && ~isempty(dirIndexa2) && ~isempty(dirIndexb2)
                ...
                if polc2_counter>=100
                    ...
                    polarize_time_c2=polarize_time_c2+t-100;
            ...
            if ~isempty(dirIndexa1) && ~isempty(dirIndexb1) && ~isempty(dirIndexa2) && ~isempty(dirIndexb2) && pol_samedir==0
                ...
                if samedir_counter>=100
                    ...
                    polarize_time_yes=polarize_time_yes+t-100;
                    num_pol_yes=num_pol_yes+1;
                    pol_samedir=1;
                end
            else
                samedir_counter=0;
            ...
            if ~isempty(dirIndexa1) && ~isempty(dirIndexb1) && ~isempty(dirIndexa2) && ~isempty(dirIndexb2) && pol_lf==0
                ...
                if xor(abs(medang1-3*pi/2)<pi/4, abs(medang2-pi/2)<pi/4)
                    ...
                    if lf_counter>=100
                        pol_lf=1;
                        num_pol_lf=num_pol_lf+1;
                        polarize_time_lf=polarize_time_lf+t-100;
                    end
    ...
    if(quit_cond==0)
        ...
        if save_matfile==1
        save(strcat('./FigureAndMovieCode/vid_matfiles/signal_switches_sides/500stepsc2_2000stepsc1/antagonism_ractorho/0_5epsilon10RhoRemoved',int2str(ppp),'.mat'),...
...
if countpol==1
    ...
    if num_pol_c1+num_pol_c2>0
        ...
        avg_steps_total=(polarize_time_c1+polarize_time_c2)/(num_pol_c1+num_pol_c2);
    end
    if num_pol_yes>0
        sprintf('Average polarization time for both cells in same direction with %d runs: %d timesteps',num_pol_yes,polarize_time_yes/num_pol_yes)
        avg_steps_samedir=polarize_time_yes/num_pol_yes;
    else
        sprintf('None polarized in the same direction')
    end
    if num_pol_lf>0
        avg_steps_lf=polarize_time_lf/num_pol_lf;
    end
