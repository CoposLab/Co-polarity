...
while (ppp<=1)
    ...
    for t=1:(Nt-1)
        ...
        kb1(boundC1_2)=1*ones(length(boundC1_2),1);
        kc1=zeros(length(b1),1);
        kc1(boundC1_2)=1*ones(length(boundC1_2),1);

        kb2_1=zeros(length(a2),1);
        kb2_1(boundC2_1)=1*ones(length(boundC2_1),1);
        kb2_2=zeros(length(a2),1);
        kb2_2(boundC2_3)=1*ones(length(boundC2_3),1);
        kc2_1=zeros(length(b2),1);
        kc2_1(boundC2_1)=1*ones(length(boundC2_1),1);
        kc2_2=zeros(length(b2),1);
        kc2_2(boundC2_3)=1*ones(length(boundC2_3),1);

        kb3_1=zeros(length(a3),1);
        kb3_1(boundC3_2)=1*ones(length(boundC3_2),1);
        kb3_2=zeros(length(a3),1);
        kb3_2(boundC3_4)=1*ones(length(boundC3_4),1);
        kc3_1=zeros(length(b3),1);
        kc3_1(boundC3_2)=1*ones(length(boundC3_2),1);
        kc3_2=zeros(length(b3),1);
        kc3_2(boundC3_4)=1*ones(length(boundC3_4),1);

        kb4=zeros(length(a4),1);
        kb4(boundC4_3)=1*ones(length(boundC4_3),1);
        kc4=zeros(length(b4),1);
        kc4(boundC4_3)=1*ones(length(boundC4_3),1);
        abmax=50;
        ...
        % rxna1 = dt*( F(a1,b1) + (a1.*(1+alpha(1)*xC1 + kb1.* (flip(b2).*(flip(b2)<=abmax) + abmax*(flip(b2)>abmax)) ) - a1.*a1)); %Cell 1 branched
        % rxnb1 = dt*( F(b1,a1) + (b1.*(1+alpha(1)*yC1 + kc1.* (flip(a2).*(flip(a2)<=abmax) + abmax*(flip(a2)>abmax)) ) - b1.*b1)); %Cell 1 bundled
        % rxna2 = dt*( F(a2,b2) + (a2.*(1+alpha(1)*xC2 + kb2_1.* (flip(b1).*(flip(b1)<=abmax) + abmax*(flip(b1)>abmax)) ...
        %     + kb2_2.* (flip(b3).*(flip(b3)<=abmax) + abmax*(flip(b3)>abmax)) ) - a2.*a2)); %Cell 2 branched
        % rxnb2 = dt*( F(b2,a2) + (b2.*(1+alpha(1)*yC2 + kc2_1.* (flip(a1).*(flip(a1)<=abmax) + abmax*(flip(a1)>abmax)) ...
        %     + kc2_2.* (flip(a3).*(flip(a3)<=abmax) + abmax*(flip(a3)>abmax)) ) - b2.*b2)); %Cell 2 bundled
        % rxna3 = dt*( F(a3,b3) + (a3.*(1+alpha(1)*xC3 + kb3_1.* (flip(b2).*(flip(b2)<=abmax) + abmax*(flip(b2)>abmax)) ...
        %     + kb3_2.* (flip(b4).*(flip(b4)<=abmax) + abmax*(flip(b4)>abmax))) - a3.*a3)); %Cell 3 branched
        % rxnb3 = dt*( F(b3,a3) + (b3.*(1+alpha(1)*yC3 + kc3_1.* (flip(a2).*(flip(a2)<=abmax) + abmax*(flip(a2)>abmax)) ...
        %     + kc3_2.* (flip(a4).*(flip(a4)<=abmax) + abmax*(flip(a4)>abmax)) ) - b3.*b3)); %Cell 3 bundled
        % rxna4 = dt*( F(a4,b4) + (a4.*(1+alpha(1)*xC4 + kb4.* (flip(b3).*(flip(b3)<=abmax) + abmax*(flip(b3)>abmax)) ) - a4.*a4)); %Cell 4 branched
        % rxnb4 = dt*( F(b4,a4) + (b4.*(1+alpha(1)*yC4 + kc4.* (flip(a3).*(flip(a3)<=abmax) + abmax*(flip(a3)>abmax)) ) - b4.*b4)); %Cell 4 bundled
