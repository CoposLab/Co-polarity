...
while (ppp<=100)
    ...
    plot(Xa,a3,'-o','markerfacecolor',[159 219 229]/255,'linewidth',3); hold on;
    plot(Xa,b3,'-ok','markerfacecolor','k','linewidth',3);
    plot(s3,xC3,'-.','color',[0 0.45 0.75],'linewidth',1);
    plot(s3,yC3,'-.k','linewidth',1);
    % xlim([0 10]); ylim([0 2]);
    %title('Time = 0');
    set(gca,'fontname','times','fontsize',20); box on;
    lgd = legend('Branched network','Bundled network','Rac','Rho','Location','northeast');
    ...
    plot(Xa,a4,'-o','markerfacecolor',[159 219 229]/255,'linewidth',3); hold on;
    plot(Xa,b4,'-ok','markerfacecolor','k','linewidth',3);
    plot(s4,xC4,'-.','color',[0 0.45 0.75],'linewidth',1);
    plot(s4,yC4,'-.k','linewidth',1);
    % xlim([0 10]); ylim([0 2]);
    %title('Time = 0');
    set(gca,'fontname','times','fontsize',20); box on;
    lgd = legend('Branched network','Bundled network','Rac','Rho','Location','northeast');
    ...
    for t=1:(Nt-1)
        ...
        % Konx2(boundC2_2)=Konx2(boundC2_2)*100;
        % Konx3(boundC3_2)=Konx3(boundC3_2)*100;
        ...
        ka1=zeros(length(a1),1);
        ka1(boundC1)=1*ones(length(boundC1),1);
        kb1=zeros(length(b1),1);
        kb1(boundC1)=1*ones(length(boundC1),1);
        ka2=zeros(length(a2),1);
        ka2(boundC2_1)=1*ones(length(boundC2_1),1);
        ka2(boundC2_2)=1*ones(length(boundC2_2),1);
        kb2=zeros(length(b2),1);
        kb2(boundC2_1)=1*ones(length(boundC2_1),1);
        kb2(boundC2_2)=1*ones(length(boundC2_2),1);
        ka3=zeros(length(a3),1);
        ka3(boundC3_1)=1*ones(length(boundC3_1),1);
        ka3(boundC3_2)=1*ones(length(boundC3_2),1);
        kb3=zeros(length(b3),1);
        kb3(boundC3_1)=1*ones(length(boundC3_1),1);
        kb3(boundC3_2)=1*ones(length(boundC3_2),1);
        ka4=zeros(length(a4),1);
        ka4(boundC4)=1*ones(length(boundC4),1);
        kb4=zeros(length(b4),1);
        ...
        rxna3 = dt*( F(a3,b3) + Ka3.*(a3.*(1+alpha(1)*xC3 + 0*ka3)) - a3.*a3); %Cell 3 branched
        rxnb3 = dt*( F(b3,a3) + Kb3.*(b3.*(1+alpha(1)*yC3 + 0*kb3)) - b3.*b3); %Cell 3 bundled
        rxna4 = dt*( F(a4,b4) + Ka4.*(a4.*(1+alpha(1)*xC4 + 0*ka4)) - a4.*a4); %Cell 4 branched
        rxnb4 = dt*( F(b4,a4) + Kb4.*(b4.*(1+alpha(1)*yC4 + 0*kb4)) - b4.*b4); %Cell 4 bundled
